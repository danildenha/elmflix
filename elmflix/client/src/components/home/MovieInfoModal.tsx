...
import { FiX, FiPlay, FiPlus, FiCheck, FiSearch } from 'react-icons/fi';
import { Movie, addToWatchHistory, addToWatchlist, removeFromWatchlist, isInWatchlist, content } from '@/lib/schema';
...
const MovieInfoModal: React.FC<MovieInfoModalProps> = ({ movie, onClose }) => {
  ...
  const handleSearch = (query: string) => {
    ...
    const results = content.filter(m => 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-zinc-900 rounded-lg shadow-lg">
        {/* Close button */}
        <button
          onClick={onClose}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {searchResults.map((result) => (
                  <div 
                    ...
                    onClick={() => {
                      onClose();
                      // You might want to open a new modal for the clicked movie
